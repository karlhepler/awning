...
3. **No rain (multi-signal gate)**: the awning closes only on **observed wetness** or a forecast signal that is not vetoed by provably-clear sky conditions. Signals are divided into two categories:
   ...
   **Observed signals** (close on any positive reading; never suppressed by any veto):
   ...
   - **RainViewer NEXRAD radar** shows precipitation over the configured `LATITUDE`/`LONGITUDE` when multiple adjacent radar pixels (at least 2 non-transparent pixels in a 3×3 window around the target location) are present. This neighborhood requirement suppresses lone-clutter-pixel false positives from NEXRAD clear-air-mode echoes (biological scatter, ground clutter, anomalous propagation); real precipitation cells light up many adjacent pixels and trivially exceed the threshold. This is a live radar observation, independent of the Open-Meteo forecast model, so it catches storms the hourly model has not yet ingested. The radar check fails open: any fetch/parse error (or a missing Pillow dependency) returns "no radar rain" so it can never wedge the awning closed. A single radar tile is sampled (no adjacent-tile lookup); see the in-code note for the tile-boundary caveat.
